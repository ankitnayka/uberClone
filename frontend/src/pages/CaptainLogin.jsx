...
const CaptainLogin = () => {
  ...
  const [loginCaptain, { data, isLoading, isSuccess, isError, error }] =
  ...
  useEffect(() => {
    ...
    if (error) {
      const inputError = error?.data?.errors?.length
    ...
    <>
      ...
      <div className="min-h-screen w-full bg-[#FFB121] flex flex-col justify-center items-center px-4 sm:px-6 md:px-10 lg:px-20">
        <div className="flex flex-col items-center justify-center w-full max-w-md bg-white p-6 rounded-lg shadow-lg">
          ...
          <Button
            className="my-3 w-full bg-blue-500 text-white py-2 rounded-lg shadow-md hover:bg-blue-600"
          ...
          <p className="text-center text-sm mt-3">
            Register as captain
            <Link to="/captain-signup" className="text-black font-bold ml-1">
              Create New Account
            </Link>
          </p>

          <Link to="/login" className="w-full">
            <Button className="my-3 w-full bg-[#333333] text-white py-2 rounded-lg shadow-md hover:bg-gray-800">
              Sign in as User
            </Button>
          </Link>
        </div>
